...
{
    csv = csv_file();
    std::vector<std::vector<std::string>> csv_data = csv.load_from_csv("./data.csv");
    for (auto line : csv_data)
    {
        Contacts ct;

        // 防止空字段导致错误
        auto safeField = [&line](size_t i) -> std::string {
            return (i < line.size()) ? line[i] : "";
        };
        std::string f0 = safeField(0), f1 = safeField(1), f2 = safeField(2);
        std::string f3 = safeField(3), f4 = safeField(4), f5 = safeField(5);

        if (!f0.empty()) ct.setId(QString::fromStdString(f0));
        if (!f1.empty()) ct.setName(QString::fromStdString(f1));
        if (!f2.empty()) ct.setPhoneNumber(QString::fromStdString(f2));
        if (!f3.empty()) ct.setType(QString::fromStdString(f3));
        if (!f4.empty()) ct.setBirth(QString::fromStdString(f4));
        if (!f5.empty()) ct.setRemarks(QString::fromStdString(f5));
...
bool contactsList::saveContacts()
{
    ...
    csv.write_to_csv("./data.csv", csv_data);
